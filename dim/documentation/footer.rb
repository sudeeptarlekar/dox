...
Dir.chdir("#{file_dir}/..") do
  ...
  remote = `git remote get-url origin`.strip.gsub(%r{//.*@}, '//') # assuming origin
  folder = `git rev-parse --show-prefix`.strip.gsub(%r{/$}, '')
  time_sha1 = `git log -n 1 --pretty="format:%cd|%h" --date=iso -- . 2>&1`.split('|')
  time = parseTime(time_sha1[0])
  sha1 = time_sha1[1]

  data['.'] = "Based on #{remote}, folder #{folder}, from #{time}, commit #{sha1}."
end
